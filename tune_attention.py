...
for handle in training_handles:
    ...
    encoder, autoencoder = make_autoencoder_model(
            (FUTURE_PERIOD_PREDICT, len(input_columns_autoenc)), len(input_columns_autoenc)
    )
    ...
    #
    #       Model Attention     -   Hyperparameter Tuning
    #
    BATCH_SIZE = 60
    N_ITER = 50
    ...
    def wrapper_attention(hp):
        attention_model = make_attention_model(
                (FUTURE_PERIOD_PREDICT, len(input_columns_autoenc)), 3, hp=hp
        )
    ...
    tuner.search(encoded_train_X, tensorflow.keras.utils.to_categorical(train_Y, num_classes=None),
                 validation_split=0.2,
                 epochs=50,
                 callbacks=[tensorflow.keras.callbacks.EarlyStopping(patience=5)])

    with open(path_to_hp_attention, 'w') as f:
        import json
